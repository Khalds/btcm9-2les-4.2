...
function getUserById(array, id) {
  const getUser = array.find((el) => el.id === id)
  console.log(getUser);
}
...
function removeAddresses(array) {
  const remAddres = array.map(function (i) {
    delete i.address
    return i
  })
  return remAddres;
...
function deleteUser(array, id) {
  const remUser = array.filter(i => i.id !== id)
  return remUser
}
...
function getUsersCompany(array, id) {
  const getCompany = array.find((el) => el.id === id);
...
//   return array;
// }
// console.log(changePhone(users, 1, '9999999'));
...
console.log(changePhone(users, 1, '89667221352'));